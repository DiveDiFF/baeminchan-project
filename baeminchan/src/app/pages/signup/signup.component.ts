...
export class SignupComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // this.http.delete(`https://server.yeojin.me/api/users/delete/<21>/`, )
    }
  ...
  inputAuthNum() {
    ...
    const phoneNum = `${this.userForm.value.phoneNumberGroup.phNum1}-${this.userForm.value.phoneNumberGroup.phNum2}-${this.userForm.value.phoneNumberGroup.phNum3}`;
    const formData: FormData = new FormData();
    formData.append('contact_phone', phoneNum);
    this.http.post(`https://server.yeojin.me/api/users/phone/`, formData).subscribe((resp) => console.log(resp));
  }
  sendAuthNum(e) {
    console.log(e);
    const phoneNum = `${this.userForm.value.phoneNumberGroup.phNum1}-${this.userForm.value.phoneNumberGroup.phNum2}-${this.userForm.value.phoneNumberGroup.phNum3}`;
    const formData: FormData = new FormData();
    formData.append('contact_phone', phoneNum);
    formData.append('auth_key', e);
    this.http.post(`https://server.yeojin.me/api/users/phone/auth/`, formData).subscribe((resp) => {
    console.log(resp);
      // if (resp.status === 400) {
      //   this.confirmAuthNum = true;
      // }
  });
    // this.sentAuthNum = false;
  }
  onSubmit() {
    this.userForm.reset();
  }
